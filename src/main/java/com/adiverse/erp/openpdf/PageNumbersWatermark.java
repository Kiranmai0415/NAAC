...
public class PageNumbersWatermark extends PdfPageEventHelper {
     ...
     */
	/*
	 * public static void main(String[] args) { try { // step 1: creating the
	 * document Document doc = new Document(PageSize.A4, 50, 50, 100, 72); // step
	 * 2: creating the writer PdfWriter writer = PdfWriter.getInstance(doc, new
	 * FileOutputStream("pageNumbersWatermark.pdf")); // step 3: initialisations +
	 ...
	 * (int k = 0; k < 10; ++k) text += text; Paragraph p = new Paragraph(text);
	 * p.setAlignment(Element.ALIGN_JUSTIFIED); doc.add(p); // step 5: closing the
	 * document doc.close(); } catch ( Exception e ) { e.printStackTrace(); } }
	 */
    ...
    public void onOpenDocument(PdfWriter writer, Document document) {
        try {
            ...
            headerImage = Image.getInstance("CMRLogo.png");

...
function ExtendedProfile() {
  ...
  useEffect(
    () =>
      fetch(
        'http://localhost:8080/api/v1/getallextendeddata?collegeId='+collegeId+'&financialYear=' +
          fyear +
  ...
  function getAuthorizeExtendedprofile() {
    ...
    fetch(
      resources.APPLICATION_URL
        + "extendeddatasubmittoprincipal?collegeId=1&finacialYear=financialYear&typeofInstitution=autonomous",
      authAPIMethod
    ).then((response) => {
      console.log(response);
      if (response.ok) {

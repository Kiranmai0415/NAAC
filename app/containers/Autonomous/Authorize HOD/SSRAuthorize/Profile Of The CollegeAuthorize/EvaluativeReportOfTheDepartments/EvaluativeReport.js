...
function EvaluativeReport() {
  ...
  useEffect(
    () =>
      fetch(
        "http://localhost:8080/api/v1/getallevaluation?collegeId=" +
          collegeId +
          "&financialYear=" +
          fYear +
          "&typeofInstitution=" +
  ...
  function getAuthorizeEvaluativereport(e) {
    ...
    fetch(
      resources.APPLICATION_URL +
        "evaluativeRepotDataSubmittoPrincipal?collegeId=1&finacialYear=financialYear&typeofInstitution=autonomous",
      authAPIMethod
    ).then((response) => {

...
import "./ValueAddedAuthorize.css";
...
function ValueAddedAuthorize() {
  ...
  function onConfirmingReject() {
    ...
    fetch(
      "http://localhost:8080/api/v1/hodDashBoardRejectRecord?screenType=valueAdded",
      rejectAPImethod
    ).then((response) => {
  ...
  function onConfirmingAuthorize(e) {
    ...
    fetch(
      "http://localhost:8080/api/v1/hodDashBoardAuthorizedEmail?screenType=valueAdded",
      authAPImethod
    ).then((response) => {
      console.log(response);
      if (response.ok) {
...
export default ValueAddedAuthorize;

...
import Criteria71 from "./ViewHOD/CriteriaView-7/cV71/c71";
import Criteria72 from "./ViewHOD/CriteriaView-7/cV72/c72";
import Criteria73 from "./ViewHOD/CriteriaView-7/cV73/c73";
...
console.log("current year==" + fYear);
  ...
  useEffect(
    () => fetch(
      'http://localhost:8080/api/v1/getTrackingInfo?collegeId='+collegeId+'&financialYear='+fYear+'&typeofInstitution='+instituteType).then((response) => response
      .json()
      .then((data) => {
  ...
  function onConfirmingReject() {
    ...
    fetch(
      resources.APPLICATION_URL
        + "deanRejectRecord?institutionType=university",
      rejectAPImethod
    ).then((response) => {
  ...
  function onClickingSaveSendingTexts() {
    ...
    fetch(
      resources.APPLICATION_URL + 'criteria7submittoprincipal?collegeId='+collegeId+'&financialYear='+fYear+'&typeofInstitution='+instituteType,
      authorizeAPImethod
    ).then((response) => {
      console.log(response);
      if (response.ok) {

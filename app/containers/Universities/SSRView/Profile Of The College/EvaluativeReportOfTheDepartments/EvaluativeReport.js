...
function EvaluativeReport() {
  ...
  useEffect(
    () => fetch(resources.APPLICATION_URL + 'getallevaluation?collegeId=' + collegeId + '&financialYear=' + fYear + '&typeofInstitution=' + instituteType).then(
      (response) => response
        .json()
        .then((data) => {
  ...
  function onClickingSave(e) {
    ...
    fetch(resources.APPLICATION_URL + "evaluationdata", filesFetchMethod).then(
      (response) => {

...
function ExecutiveSummary() {
  ...
  const { instituteType } = AuthService.getCurrentUser();
  console.log("instituteType=======>" + instituteType);
  const { collegeId } = AuthService.getCurrentUser();
  console.log("eamcetCode==========>" + collegeId);
  const d = new Date();
  const currYear = d.getFullYear();
  console.log("var ==>" + currYear);
  const fYear = parseInt(currYear, 10) - 1 + "-" + currYear;
  console.log("current year==" + fYear);

  // get call
  const financialYear = "2021-2022";
  useEffect(
    () => fetch(resources.APPLICATION_URL + 'getallexecutivedata?collegeId=' + collegeId + '&financialYear=' + fYear + '&typeofInstitution=' + instituteType).then(
      (response) => response
        .json()
        .then((data) => {
      ...
      <form>
        ...
        <div className="executive-eachTable-container">
          <div style={{ margin: "10px" }}>
            <label htmlFor="es1">1.Introductory Note on the Institution fdgfdg</label>
            <textarea readOnly
              ...
              {pathValuees6 === "" ? null : (
                <div className="c21-fileButtons-container" >
                  ...
                  <button className="c11-removeFileButton-style"
                    onClick={(e) => {
                      setPathValuees6("");
                      document.getElementById("es6").value = "";

...
function EvaluativeReport() {
  ...
  const [pathValuef4, setPathValuef4] = useState("");
   ...
   useEffect(
    () => fetch('http://localhost:8080/api/v1/evaluativedata').then(
      (response) => response
        .json()
        .then((data) => {
          console.log(JSON.stringify(data));
          setValuet1(data[0].evaluativeReport[0].valuet1);
          setValuet2(data[0].evaluativeReport[1].valuet1);
          setValuet3(data[0].evaluativeReport[2].valuet1);
          setValuet4(data[0].evaluativeReport[3].valuet1);
        })
        .catch((error) => {

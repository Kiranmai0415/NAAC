...
function ExecutiveSummary() {
  ...
  useEffect(
    () =>
      fetch(
        'http://localhost:8080/api/v1/getallexecutivedata?collegeId='+collegeId+'&financialYear='+fYear+'&typeofInstitution='+instituteType,
      ).then((response) =>
        response
          .json()
          .then((data) => {
  ...
  function getAuthorizeExecutive() {
    ...
    fetch(
      resources.APPLICATION_URL
        + "executiveDatasubmittoprincipal?collegeId=1&finacialYear=financialYear&typeofInstitution=autonomous",
      authAPIMethod
    ).then((response) => {

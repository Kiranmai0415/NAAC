...
function EvaluativeReport() {
  ...
  useEffect(
    () =>
      fetch("http://localhost:8080/api/v1/evaluativedata").then((response) =>
        response
          .json()
          .then((data) => {
  ...
  function getAuthorizeEvaluativereport(e) {
    ...
    fetch(
      resources.APPLICATION_URL +
        "evaluationDatasubmittoprincipal?collegeId=1&finacialYear=financialYear&typeofInstitution=autonomous",
      authAPIMethod
    ).then((response) => {
